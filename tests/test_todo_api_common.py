...
def test_json_content_type(test_client: TestClient, auth_headers: dict):
...
def test_empty_database_returns_empty_array(
...
def test_invalid_json_payload(test_client: TestClient, auth_headers: dict):
...
def test_method_not_allowed(client: FlaskClient, auth_headers: dict):
    """許可されていないHTTPメソッドに対して適切なエラーが返されることを確認"""
    # DELETE /todosはAPI仕様に含まれていない
    response = client.delete("/api/v1/todos/", headers=auth_headers)
...
def test_unauthorized_access(test_client: TestClient):
    """認証なしでアクセスした場合に401エラーが返されることを確認"""
